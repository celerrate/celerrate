...
| --- | --- | --- |
| CEL0050 | a recorded entry no longer matches any current finding (the code was fixed, the enclosing method was renamed, or an engine upgrade reworded the message) | re-record with `celerrate check --baseline` to refresh the file |
